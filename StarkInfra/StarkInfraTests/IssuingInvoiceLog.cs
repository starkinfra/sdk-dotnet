...
{
    public class IssuingInvoiceLogTest
    {
        ...
        public void QueryAndGet()
        {
            ...
            IssuingInvoice.Log getLog = IssuingInvoice.Log.Get(id: logs.First().ID);
        ...
        public void Page()
        {
            ...
            List<IssuingInvoice.Log> page;
            string cursor = null;
            for (int i = 0; i < 2; i++)
            {
                (page, cursor) = IssuingInvoice.Log.Page(limit: 2, cursor: cursor);
                foreach (IssuingInvoice.Log entity in page)
                {
